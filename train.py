...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    LAMBDA_NORMAL_START_ITER = opt.lambda_normal_start_iter
    LAMBDA_NORMAL_END_ITER = opt.lambda_normal_end_iter
    ...
    while iteration < TOT_ITER:        
        ...
        # 2dgs loss
        if iteration > LAMBDA_NORMAL_START_ITER and iteration < LAMBDA_NORMAL_END_ITER:
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                loss_dict = {
                    ...
                    "distort": f"{ema_dist_for_log:.{7}f}",
                    "normal": f"{ema_normal_for_log:.{7}f}",
...
def prepare_output_and_logger(args):    
    if not args.model_path:
        ...
        if args.project_name:
            args.model_path = os.path.join(args.model_path, args.project_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[3000 * x for x in range(1, 61)])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3000 * x for x in range(1, 61)])
